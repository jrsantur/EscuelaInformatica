...
public class MetodoLinealMixto extends ActionBarActivity {
    private EditText txtr0, txtnAleat, txtm, txta, txtc;
    private TableLayout tabla;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_metodo_lineal_mixto);
        txtr0=(EditText)findViewById(R.id.txtr0LinMixto);
        txta=(EditText)findViewById(R.id.txtMultiplicativoLinMixto);
        txtc=(EditText)findViewById(R.id.txtAditivoLinMixto);
        txtm=(EditText)findViewById(R.id.txtModuloLinMixto);
        txtnAleat=(EditText)findViewById(R.id.txtNAleatLinMixto);
        tabla=(TableLayout)findViewById(R.id.tablaNroAleatLinMixto);
    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.menu_metodo_lineal_mixto, menu);
        return true;
    }
    ...
    public void btnGenerarNrosAleatLinMixto(View view){
        try{
            if(txtr0.getText()!=null && txta.getText()!=null && txtm.getText()!=null && txtnAleat.getText()!=null) {
                ...
                int [] arr=metodoLinealMixto(r0,a,c,m,nnumeros);
                agregarcabecera();
                agregarcelda(nnumeros,arr,m);
            }
        }catch (Exception ex){
            Toast notificacion=Toast.makeText(this,"Ingrese datos correctamente"+ex,Toast.LENGTH_LONG);
            notificacion.show();
        }
    }

    private int []  metodoLinealMixto(int r0, int a, int c, int m,int n){
        int [] serie=new int[n];
        serie[0]=((a*r0)+c)%m;
        for(int i=1;i<n;i++){
            serie [i]=(c+(a*serie [i-1]))%m;
        }
        return serie;
    }
